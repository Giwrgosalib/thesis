...
class oauth2api(object):
    ...
    def generate_user_authorization_url(self, env_type, scopes, state=None):
        ...
        scopes_str = ' '.join(scopes)  # Renamed to avoid conflict with scopes parameter
        param = {
                'client_id':credential.client_id,
                'redirect_uri':credential.ru_name,
                'response_type':'code',
                'prompt':'login',
                'scope':scopes_str  # Use the renamed variable
                }
        ...
        query = urllib.parse.urlencode(param)  # Python 3
    ...
    def get_application_token(self, env_type, scopes):
        ...
        credential = credentialutil.get_credentials(env_type)       
        headers = model_util._generate_request_headers(credential) 
        body = model_util._generate_application_request_body(credential, ' '.join(scopes))    
    ...
    def exchange_code_for_access_token(self, env_type, code):       
        ...
        headers = model_util._generate_request_headers(credential)
        body = model_util._generate_oauth_request_body(credential, code)
        resp = requests.post(env_type.api_endpoint, data=body, headers=headers)
            
        content = json.loads(resp.content)
        token = oAuth_token()     
        token.token_response = content  # ADD THIS LINE to store the raw response
        ...
        if resp.status_code == requests.codes.ok:
            ...
            token.error = str(resp.status_code) + ': ' + content.get('error_description', content.get('error', 'Unknown error'))  # Safer access
            logging.error("Unable to retrieve token.  Status code: %s - %s", resp.status_code, requests.status_codes._codes[resp.status_code])
            logging.error("Error: %s - %s", content.get('error', 'N/A'), content.get('error_description', 'N/A'))  # Safer access
        return token
    ...
    def get_access_token(self, env_type, refresh_token, scopes):
        ...
        headers = model_util._generate_request_headers(credential)
        body = model_util._generate_refresh_request_body(' '.join(scopes), refresh_token)
        resp = requests.post(env_type.api_endpoint, data=body, headers=headers)
        content = json.loads(resp.content)
        token = oAuth_token()        
