...
def _generate_request_headers(credential):
    ...
    # Fix: Convert string to bytes before encoding
    auth_string = (credential.client_id + ':' + credential.client_secret).encode('utf-8')
    b64_encoded_credential = base64.b64encode(auth_string)
    
    # Fix: Decode bytes to string for the header
    headers['Authorization'] += b64_encoded_credential.decode('utf-8')
